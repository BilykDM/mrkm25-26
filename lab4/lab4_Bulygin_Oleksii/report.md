...
## Аналіз безпеки незаперечних цифрових підписів
...
**Висновок:** Протокол заперечення є одночасно обґрунтованим та повним.

## 4. Зведена таблиця результатів

| Категорія атаки | Результат | Основа безпеки |
|-----------------|-----------|----------------|
| Повний перебір DLP | ЗАХИЩЕНО* | Розмір ключа (≥1024 біт) |
| Підробка транскриптів | ЗА ЗАДУМОМ | Непередаваність |
| Підробка підписів | ЗАХИЩЕНО | Складність DLP |
| Маніпуляція викликами | ЗАХИЩЕНО | Випадкові виклики |
| Витяг приватного ключа | ЗАХИЩЕНО | Складність DLP |
| Зловживання запереченням | ЗАХИЩЕНО | Обґрунтованість та повнота |

\* Малі ключі (<32 біт) вразливі до повного перебору

## 5. Ключові властивості безпеки

### 5.1. Непідробність (Unforgeability)
Без приватного ключа підписи неможливо підробити. Базується на складності проблеми дискретного логарифма.

### 5.2. Непередаваність (Non-transferability)
Транскрипти верифікації не можуть переконати третіх сторін. Будь-хто може підробити транскрипти, що "виглядають дійсними". Це ОСОБЛИВІСТЬ, а не вада — захищає приватність підписувача.

### 5.3. Заперечуваність (Deniability)
Підписувач може довести, що підпис НЕ його. Протокол заперечення є обґрунтованим (неможливо заперечити дійсний) та повним (завжди можна заперечити недійсний).

### 5.4. Інтерактивна верифікація
Підписи можуть бути верифіковані лише через пряму взаємодію з підписувачем.

## 6. Рекомендовані розміри ключів

| Рівень безпеки | Розмір ключа | Застосування |
|----------------|--------------|--------------|
| Мінімальний | 1536 біт | Застарілі системи |
| Стандартний | 2048 біт | Загальне використання |
| Високий | 3072 біт | Чутливі застосування |
| Дуже високий | 4096 біт | Довгострокова безпека |
